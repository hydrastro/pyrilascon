"""The vocabulary (axes) of the ASCON design space.

Self-contained as of stage 2: the enums live here (previously re-exported from
``ascon_arch.enums``, now deleted). These are the coordinates a design point is
built from, plus the two canonical algorithm sets.
"""
...
class AlgorithmFeature(str, Enum):
    # NIST SP 800-232 family implemented + KAT-verified in the golden model.
    AEAD128 = "aead128"
    HASH256 = "hash256"
    XOF128 = "xof128"
    CXOF128 = "cxof128"

    # Design-space placeholders (need golden-model/KAT support before they are
    # production-verified algorithm targets).
    AEAD128A = "aead128a"
    AEAD80PQ = "aead80pq"
    HASH = "hash"
    HASHA = "hasha"
...
# The eight single-algorithm targets swept by the design space. The four "a"/pq
# variants are placeholders (no golden-model KAT yet).
...
# Functional families the golden model verifies against NIST KATs. Both the
# generic names used by the enumeration (HASH/XOF/CXOF) and the NIST-specific
# names (HASH256/XOF128/CXOF128) are included, so downstream classification is
# robust to the historical naming split.
KAT_BACKED_ALGORITHMS: frozenset[AlgorithmFeature] = frozenset(
    {
        AlgorithmFeature.AEAD128,
        AlgorithmFeature.HASH,
