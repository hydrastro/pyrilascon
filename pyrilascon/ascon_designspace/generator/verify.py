...
def _aead128_axis_vectors(seed: int = 0xA715) -> list[dict]:
    """Encrypt, decrypt round-trip, and corrupted-tag cases for the AXIS engine,
    from the verified model, spanning empty/partial/aligned/multi-block."""
    import random

    from ascon_hwmodel.aead_encrypt import aead_encrypt
    from ascon_designspace.generator.aead import aead_variant_params

    enum = aead_variant_params("aead128")["enum"]
    rng = random.Random(seed)
    rb = lambda n: bytes(rng.getrandbits(8) for _ in range(n))
    key, nonce = rb(16), rb(16)
    lengths = [(0, 0), (0, 1), (1, 0), (3, 5), (8, 8), (16, 16), (15, 17),
               (0, 16), (16, 0), (32, 32), (7, 31), (31, 7)]
    vecs = []
    for al, ml in lengths:
        ad, pt = rb(al), rb(ml)
        r = aead_encrypt(key, nonce, ad, pt, enum)
        ct, tag = bytes(r.ciphertext), bytes(r.tag)
        # encrypt
        vecs.append(dict(key=key, nonce=nonce, ad=ad, inp=pt, dec=False, tag_in=b"\x00" * 16,
                         exp_out=ct, exp_tag=tag, exp_valid=True))
        # decrypt round-trip (valid tag)
        vecs.append(dict(key=key, nonce=nonce, ad=ad, inp=ct, dec=True, tag_in=tag,
                         exp_out=pt, exp_tag=b"\x00" * 16, exp_valid=True))
    # one corrupted-tag rejection
    ad, pt = rb(8), rb(8)
    r = aead_encrypt(key, nonce, ad, pt, enum)
    ct, tag = bytes(r.ciphertext), bytes(r.tag)
    bad = bytes([tag[0] ^ 0x01]) + tag[1:]
    vecs.append(dict(key=key, nonce=nonce, ad=ad, inp=ct, dec=True, tag_in=bad,
                     exp_out=pt, exp_tag=b"\x00" * 16, exp_valid=False))
    return vecs


def verify_aead128_axis(ad_max: int = 32, msg_max: int = 32, **kw) -> tuple[bool, int, str]:
    """Stream-native AXI-Stream AEAD128 engine vs the model (enc/dec/tag-reject)."""
    from ascon_designspace.generator.aead import emit_aead128_core
    from ascon_designspace.generator.axis import emit_aead128_axis, emit_aead128_axis_testbench

    vecs = _aead128_axis_vectors()
    return _run_sim_files(
        [("ascon_aead128_axis", emit_aead128_axis(ad_max, msg_max)),
         ("ascon_aead128_core", emit_aead128_core(ad_max, msg_max)),
         ("ascon_perm_iter_r1", emit_iterative_permutation(1))],
        emit_aead128_axis_testbench(vecs, ad_max, msg_max),
        **kw,
    )


def verify_aead128_axis_mmio(ad_max: int = 32, msg_max: int = 32, **kw) -> tuple[bool, int, str]:
    """Driver-compatible MMIO peripheral vs the model, driven through the exact
    register protocol the firmware uses (enc/dec/tag-reject)."""
    from ascon_designspace.generator.aead import emit_aead128_core
    from ascon_designspace.generator.axis import (
        emit_aead128_axis, emit_aead128_axis_mmio, emit_aead128_axis_mmio_testbench)

    vecs = _aead128_axis_vectors()
    return _run_sim_files(
        [("ascon_aead128_axis_mmio", emit_aead128_axis_mmio(ad_max, msg_max)),
         ("ascon_aead128_axis", emit_aead128_axis(ad_max, msg_max)),
         ("ascon_aead128_core", emit_aead128_core(ad_max, msg_max)),
         ("ascon_perm_iter_r1", emit_iterative_permutation(1))],
        emit_aead128_axis_mmio_testbench(vecs, ad_max, msg_max),
        **kw,
    )


def verify_aead128_wb(ad_max: int = 32, msg_max: int = 32, **kw) -> tuple[bool, int, str]:
    """The Wishbone/XBUS adapter (rtl/soc/ascon_aead128_wb.v) + the MMIO datapath,
    driven exactly the way NEORV32's XBUS drives it (pulsed strobe, held cyc, ack
    in the following cycle), replaying the driver protocol vs the model."""
    import pathlib

    from ascon_designspace.generator.aead import emit_aead128_core
    from ascon_designspace.generator.axis import (
        emit_aead128_axis, emit_aead128_axis_mmio, emit_aead128_wb_xbus_testbench)

    root = pathlib.Path(__file__).resolve().parents[2]
    adapter = (root / "rtl" / "soc" / "ascon_aead128_wb.v").read_text()
    vecs = _aead128_axis_vectors()
    return _run_sim_files(
        [("ascon_aead128_wb", adapter),
         ("ascon_aead128_axis_mmio", emit_aead128_axis_mmio(ad_max, msg_max)),
         ("ascon_aead128_axis", emit_aead128_axis(ad_max, msg_max)),
         ("ascon_aead128_core", emit_aead128_core(ad_max, msg_max)),
         ("ascon_perm_iter_r1", emit_iterative_permutation(1))],
        emit_aead128_wb_xbus_testbench(vecs, ad_max, msg_max),
        **kw,
    )
