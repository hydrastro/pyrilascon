...
RTL generator grown from that model. FPGA targets are the Tang Nano 9K and 20K
(open Gowin flow); the ASIC path feeds TinyTapeout via
...
With [Nix](https://nixos.org) (flakes enabled), one command provides the entire
toolchain — Python, Icarus Verilog, gcc, the RISC-V firmware toolchain, and the
open Gowin FPGA flow (yosys, nextpnr, gowin_pack, openFPGALoader):

```sh
nix develop            # or run `direnv allow` once, if you use direnv
make test              # full suite: model + generator + RTL sim + C
make catalog           # design-space tier breakdown
make flash BOARD=tangnano20k TARGET=perm_smoke   # a generated core onto a Tang Nano
```

`nix flake check` runs the same suite in a sandbox. Without Nix you need Python
3.11+, plus (optionally) Icarus Verilog and a C compiler for the sim/C tests, and
the open Gowin flow for flashing.
