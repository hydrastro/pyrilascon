...
/*
 * Base address of the accelerator's register block.
 *
 * On the NEORV32 SoC (rtl/soc/neorv32_ascon_soc.vhd) the accelerator sits on the
 * external bus (XBUS), so this MUST be an address that NEORV32 routes to XBUS -
 * i.e. outside IMEM (0x00000000+), DMEM (0x80000000+), and the internal IO region
 * (0xFFE00000-0xFFFFFFFF). 0x90000000 satisfies that. (The ascon_aead128_wb
 * adapter decodes only the low byte, so any XBUS address + the register offsets
 * reaches the peripheral.) Override with -DASCON_ACCEL_BASE_ADDR=... for a
 * different map.
 */
#ifndef ASCON_ACCEL_BASE_ADDR
#define ASCON_ACCEL_BASE_ADDR 0x90000000u
#endif
