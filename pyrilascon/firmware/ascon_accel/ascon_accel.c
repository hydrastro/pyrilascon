...
ascon_accel_status_t ascon_accel_encrypt(
  ...
  /* The accelerator is a START-first stream engine: it must be started (so it
     enters its receive state and asserts s_tready) BEFORE payload beats are
     streamed. Streaming first would stall the initial DATA_IN_CTRL write on
     s_tready and time out the bus. Applies to both data planes. */
...
ascon_accel_status_t ascon_accel_decrypt(
  ...
  /* START-first (see encrypt): start the engine before streaming ciphertext. */
...
ascon_accel_status_t ascon_accel_hash_or_xof(
  ...
  /* START-first (see encrypt): start the engine before streaming. */
