#!/usr/bin/env python3
"""Run one buffered AXI-stream AEAD128 decrypt RTL vector against the Python oracle.

The script generates a vector-specific Verilog testbench, builds it with Icarus
Verilog, runs it with vvp, and compares emitted plaintext/authentication status
against ``ascon_hwmodel.aead_stream.axis_aead128_decrypt``.

Use ``--dry-run`` when a simulator is not available; it still emits the golden
reference values, ciphertext input beats, expected tag, and expected plaintext
release behavior.  By default the expected tag is the correct encryption tag.
Pass ``--corrupt-tag`` to verify that the RTL suppresses plaintext and reports
...
@dataclass(frozen=True)
class RtlDecryptResult:
    plaintext_hex: str
    generated_tag_hex: str
    tag_valid: int
    error: int
    error_code: int
    cycles: int
...
@dataclass(frozen=True)
class DecryptComparisonResult:
    golden: GoldenDecryptVector
    rtl: RtlDecryptResult | None
    matched: bool | None
    simulator: str | None
...
def corrupt_tag(tag: bytes) -> bytes:
    if len(tag) != DATA_BYTES:
        raise ValueError("tag must be 16 bytes")
    corrupted = bytearray(tag)
    corrupted[0] ^= 0x01
...
def build_golden_vector(
    ...
    *,
    ...
    corrupt_expected_tag: bool = False,
) -> GoldenDecryptVector:
    ...
    encrypted = axis_aead128_encrypt(
        key=key,
        nonce=nonce,
        ad_beats=ad_beats,
    ...
    tag = encrypted.tag if expected_tag is None else expected_tag
    if corrupt_expected_tag:
        tag = corrupt_tag(tag)
    decrypted = axis_aead128_decrypt(
        key=key,
        nonce=nonce,
        ad_beats=ad_beats,
        ciphertext_beats=encrypted.ciphertext_beats,
        ad_len=len(associated_data),
        text_len=len(encrypted.ciphertext),
        tag=tag,
        bus_bytes=DATA_BYTES,
    )
    return GoldenDecryptVector(
        key_hex=key.hex(),
        nonce_hex=nonce.hex(),
        associated_data_hex=associated_data.hex(),
        ciphertext_hex=encrypted.ciphertext.hex(),
        expected_tag_hex=tag.hex(),
        computed_tag_hex=decrypted.computed_tag.hex(),
        valid=decrypted.valid,
        plaintext_hex=plaintext.hex(),
        released_plaintext_hex=decrypted.plaintext.hex(),
        ad_beats=[beat_to_json(beat) for beat in ad_beats],
        ciphertext_beats=[beat_to_json(beat) for beat in encrypted.ciphertext_beats],
        plaintext_beats=[beat_to_json(beat) for beat in decrypted.plaintext_beats],
    )
...
def generate_testbench(vector: GoldenDecryptVector, *, timeout_cycles: int = 7000) -> str:
    send_lines = [send_beat_statement(beat) for beat in vector.ad_beats]
    send_lines.extend(send_beat_statement(beat) for beat in vector.ciphertext_beats)
    send_body = "\n".join(send_lines) if send_lines else "    // Zero-length AD and ciphertext: no AXI input beats."
    ...
    tag_lit = int_literal_from_bytes_le(bytes.fromhex(vector.expected_tag_hex))
    ad_len = len(bytes.fromhex(vector.associated_data_hex))
    text_len = len(bytes.fromhex(vector.ciphertext_hex))

    return f"""`timescale 1ns/1ps

module tb_ascon_aead128_stream_decrypt_buffered;
  localparam integer DATA_BYTES = 16;
  localparam integer DATA_WIDTH = DATA_BYTES * 8;
  localparam integer MAX_TEXT_BYTES = 1024;
  ...
  reg [127:0] expected_tag_i = 128'h{tag_lit};
  ...
  always @(posedge clk_i) begin
    ...
    if (done_o) begin
      $display("DONE cycle=%0d tag_valid=%0d error=%0d error_code=%0d tag=%032x", cycle_count, tag_valid_o, error_o, error_code_o, generated_tag_o);
      $finish;
    end
...
def parse_rtl_stdout(stdout: str) -> RtlDecryptResult:
    plaintext = bytearray()
    ...
    return RtlDecryptResult(
        plaintext_hex=bytes(plaintext).hex(),
        generated_tag_hex=tag.hex(),
        tag_valid=int(str(done["tag_valid"]), 0),
...
def run_iverilog_simulation(repo_root: Path, testbench_text: str, workdir: Path) -> RtlDecryptResult:
    if shutil.which("iverilog") is None or shutil.which("vvp") is None:
        raise RuntimeError("iverilog and vvp are required for RTL simulation")

    tb_path = workdir / "tb_ascon_aead128_stream_decrypt_buffered.v"
    simv_path = workdir / "tb_ascon_aead128_stream_decrypt_buffered.vvp"
...
def run_vector(
    *,
    ...
    golden = build_golden_vector(
        ...
        corrupt_expected_tag=corrupt_expected_tag,
    )
    tb = generate_testbench(golden)
    sim = simulator_available()
    if dry_run:
        return DecryptComparisonResult(golden=golden, rtl=None, matched=None, simulator=sim)
    if sim is None:
        raise RuntimeError("iverilog and vvp are required for RTL simulation; use --dry-run for golden data only")

    if workdir is None:
        with tempfile.TemporaryDirectory(prefix="ascon_stream_decrypt_sim_") as tmp:
    ...
    if golden.valid:
        matched = (
            ...
            and rtl.tag_valid == 1
            and rtl.plaintext_hex == golden.released_plaintext_hex
            and rtl.generated_tag_hex == golden.computed_tag_hex
        )
    else:
        matched = (
            rtl.error == 1
            and rtl.error_code == ASCON_ERROR_TAG_INVALID
            and rtl.tag_valid == 0
            and rtl.plaintext_hex == ""
            and rtl.generated_tag_hex == golden.computed_tag_hex
        )
    return DecryptComparisonResult(golden=golden, rtl=rtl, matched=matched, simulator=sim)


def result_to_jsonable(result: DecryptComparisonResult) -> dict[str, object]:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--plaintext-hex", default="", help="plaintext bytes as hex; the tool encrypts it to form ciphertext input")
    parser.add_argument("--expected-tag-hex", default=None, help="override the expected tag supplied to the RTL decrypt backend")
    parser.add_argument("--corrupt-tag", action="store_true", help="flip one bit in the correct tag and require plaintext suppression")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    try:
        ...
        expected_tag = (
            None
            if args.expected_tag_hex is None
            else parse_hex_bytes(args.expected_tag_hex, field="--expected-tag-hex", expected_len=16)
        )
        result = run_vector(
            key=key,
            nonce=nonce,
            ...
            plaintext=plaintext,
            expected_tag=expected_tag,
            corrupt_expected_tag=args.corrupt_tag,
