...
This board target now also contains a local open-source Gowin build path for two
NEORV32 top-levels:

```sh
# CPU + UART bootloader smoke image, no ASCON CFS
make bitstream-plain RISCV_PREFIX=riscv32-none-elf-

# CPU + UART bootloader + ASCON CFS
make bitstream-ascon RISCV_PREFIX=riscv32-none-elf-
```

The generated bitstreams are written under:

```text
build/neorv32_stream_axis_mmio/board/plain/
build/neorv32_stream_axis_mmio/board/ascon/
```

The preferred bring-up order is:

```sh
make tools RISCV_PREFIX=riscv32-none-elf-
make bitstream-plain RISCV_PREFIX=riscv32-none-elf-
sudo make prog-sram TOP_MODE=plain

make bitstream-ascon RISCV_PREFIX=riscv32-none-elf-
sudo make prog-sram TOP_MODE=ascon
make upload RISCV_PREFIX=riscv32-none-elf- SERIAL=/dev/serial/by-id/...
```

The top-level uses the board's 27 MHz oscillator, the existing active-low reset
button used by the full-slow target, the six active-low LEDs, and the onboard
BL702 USB-UART pins. If the bootloader does not print on UART after a successful
plain bitstream build, try the other Sipeed serial interface and then try
swapping `uart_tx` / `uart_rx` in `tangnano9k_neorv32_stream_axis_mmio.cst`.
