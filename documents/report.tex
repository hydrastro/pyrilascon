...
\usepackage{iftex}
\ifPDFTeX
  % pdfLaTeX needs explicit 8-bit font and input encodings.
  \usepackage[T1]{fontenc}
  \usepackage[utf8]{inputenc}
\fi
% XeLaTeX / LuaLaTeX are natively UTF-8 and supply T1-equivalent output,
% so they skip the two packages above. The document compiles under all
% three engines; run it twice (or via latexmk) so the table of contents
...
\subsection{The DMA-Fed AXI-Stream Front-End}
\label{sec:dma}

The AXI-MMIO bridge above makes bring-up simple but keeps the CPU in the
data path: firmware pushes and pops every $128$-bit beat, so a payload
of $L$ bytes costs $O(L)$ loads and stores, and the amount of in-flight
ciphertext is bounded by the small bridge RX FIFO (four output beats by
default). The autonomous front-end \texttt{ascon\_axis\_dma.v} removes
the CPU from the data path. Firmware programs a compact descriptor ---
associated-data and plaintext source addresses, their lengths, and a
destination address --- and sets a single \texttt{GO} bit; the engine
then fetches the payload from memory over a master port, feeds the
stream-native backend, drains each ciphertext beat, and writes it back
to the destination buffer, raising \texttt{DONE} (or an interrupt) when
the transfer completes.

The control plane is deliberately left untouched: key, nonce, lengths,
mode, \texttt{CONTROL.START}, and the resulting tag are still driven
through the frozen ASCON CSR ABI exactly as before, so the core
decoupling of NFR-006 is preserved --- the DMA block carries no
NEORV32-specific ports and sits only in front of the backend's
AXI-stream ports. The descriptor block is a separate MMIO window of
eight word-aligned registers (\texttt{AD\_ADDR}, \texttt{AD\_LEN},
\texttt{TEXT\_ADDR}, \texttt{TEXT\_LEN}, \texttt{DST\_ADDR},
\texttt{CTRL} carrying \texttt{GO}/\texttt{IRQ\_EN}/\texttt{CLEAR},
\texttt{STATUS} carrying \texttt{BUSY}/\texttt{DONE}/\texttt{ERROR} plus
the output-beat count, and \texttt{OUT\_BYTES}).

The one subtlety is back-pressure. The encrypt backend gates its text
input \texttt{tready} on its output not being valid, so a fresh
plaintext beat may be presented only after the previous ciphertext beat
has been accepted. The DMA engine honours this with a single in-flight
token: after presenting an input beat it stages the next one only once
the matching output beat has been written back. Exactly one beat is ever
in flight, so there is no internal output FIFO to overflow and the
streaming behaviour is independent of payload length --- precisely the
bound the CPU bridge cannot escape. Per-segment \texttt{tlast} is
asserted combinationally from the running byte counts on the final beat
of the associated-data segment and, separately, of the text segment,
matching the backend's length checks; an earlier revision latched the AD
\texttt{tlast} one cycle too late and tripped a spurious length error on
the first beat, which driving it directly from the counts resolved.

The front-end is verified by an Icarus Verilog cosimulation
(\texttt{tools/run\_stream\_axis\_dma\_system\_vector.py}) that wraps the
CSR control plane and the descriptor block around the real backend RTL,
backed by an embedded synchronous memory model. The testbench programs
the control plane and the descriptor exactly as firmware would, lets the
DMA move the payload, and compares the ciphertext read back from memory
and the tag read from the CSR registers against the same Python golden
model used throughout the verification methodology (§\ref{sec:ver}). The
matrix passes $14$ of $14$ vectors --- empty payloads, short partial
final blocks, multi-beat associated data, and multi-beat plaintext up to
the backend's $1024$-byte ($64$-beat) maximum. Reported cycle counts
scale linearly with payload length (about $82$ cycles for an empty
payload to roughly $1\,170$ for $1024$ bytes), confirming true
beat-by-beat streaming; the $64$-beat case is sixteen times the CPU
bridge's four-beat FIFO and could not be driven through the bridge
without an interleaved firmware pump. A descriptor-oriented firmware
driver (\texttt{ascon\_accel\_axis\_dma\_transport.\{c,h\}}) programs the
descriptor, launches the transfer, and polls \texttt{STATUS} for
completion; it is distinct from the beat-by-beat MMIO transport and is
compiled and unit-checked alongside the rest of the firmware. The full
register map and verification notes live in
\texttt{docs/stream\_axis\_dma\_system.md}.

\warn{The DMA front-end is verified at RTL through cosimulation against
the golden model, not yet on silicon --- like the integrated bitstream
it still awaits a board build (§\ref{sec:silicon}). It also automates
\emph{encryption} only: buffered authenticated decryption keeps the
CPU-driven transport, because its burst-at-the-end dataflow differs from
the streaming encrypt path, so decrypt-side DMA remains future work.}
...
\subsection{DMA Front-End Per-Payload Sweep (RASD~§8.4 Sizes)}
\label{sec:dma-sweep}

Table~\ref{tab:sweep} stops at the eight shapes the \emph{bounded}
CPU-driven MMIO backend supports: the stream wrapper caps
\texttt{MAX\_TEXT\_BYTES} and the bridge RX FIFO holds only four output
beats, so the $64$/$256$/$1024$-byte payloads the RASD~§8.4 also asks
for cannot be driven through the bridge without an interleaved firmware
pump. The DMA-fed front-end of §\ref{sec:dma} streams them directly. The
same Icarus system cosimulation, scripted across the RASD sizes by
\texttt{tools/run\_stream\_axis\_dma\_system\_sweep.py}, verifies every
result bit-for-bit against the Python golden model and reports the
end-to-end cycle count from the descriptor \texttt{GO} pulse to
\texttt{STATUS.DONE} (Table~\ref{tab:dma-sweep}).

\begin{table}[H]
\centering
\caption{DMA front-end per-payload sweep under Icarus co-simulation.
``Beats'' counts $128$-bit associated-data plus text beats; ``cycles''
is measured from \texttt{GO} to \texttt{STATUS.DONE}. The cosimulation
memory model accepts every request with a single-cycle read latency, so
these are the front-end's own streaming and bookkeeping cycles ---
\emph{not} a silicon or DRAM-timing measurement, and \emph{not}
comparable to the accelerator-core HW columns of Table~\ref{tab:sweep}.
All six cases match the golden model.}
\label{tab:dma-sweep}
\small
\begin{tabular}{lrrrrr}
\toprule
\rowcolor{tabhead!20}
\textbf{Case} & \textbf{AD} & \textbf{PT} & \textbf{Beats} &
\textbf{Cycles} & \textbf{cy/beat}\\
              & (B) & (B) & & (GO$\,\to\,$DONE) & \\
\midrule
\texttt{empty}        &  0 &    0 &  0 &    $82$  & ---\\
\texttt{pt16}         &  0 &   16 &  1 &    $98$  & $98.0$\\
\texttt{pt64}         &  0 &   64 &  4 &   $150$  & $37.5$\\
\texttt{pt256}        &  0 &  256 & 16 &   $354$  & $22.1$\\
\texttt{pt1024}       &  0 & 1024 & 64 & $1\,170$ & $18.3$\\
\texttt{ad16\_pt1024} & 16 & 1024 & 65 & $1\,188$ & $18.3$\\
\bottomrule
\end{tabular}
\end{table}

Two points are worth recording. First, the per-beat cost amortises a
fixed setup overhead --- descriptor latch, the initialisation $p[12]$,
and tag finalisation --- and settles around $18$ cycles per $128$-bit
beat at the larger sizes; adding one associated-data beat to the
$1024$-byte case costs $18$ more cycles, matching that slope. Second,
the $64$-beat ($1024$-byte) case is sixteen times the four-beat bridge
FIFO and completes with exactly one beat in flight throughout --- the
property the CPU bridge cannot provide, and the reason the front-end
exists. The on-silicon counterpart of these numbers, like the integrated
bitstream, still awaits a board build (§\ref{sec:silicon}).
...
\begin{enumerate}[noitemsep]
  \item \textbf{Per-payload sweep numbers on real silicon.} The
        integrated cosim numbers are captured for the bounded MMIO
        backend (Table~\ref{tab:sweep}), and the larger RASD~§8.4
        sizes ($64$/$256$/$1024$\,B) are now measured through the
        DMA-fed front-end cosim (Table~\ref{tab:dma-sweep}). What
        remains is the on-silicon counterpart: one Kintex run of the
        same payload set, pasted alongside the co-simulation tables.
...
# utilisation table matches section 6.2 above; nextpnr halts at utilisation limit
\end{lstlisting}
...
1.1 & \today & Added the autonomous descriptor-driven DMA-fed AXI-stream front-end --- RTL engine and system wrapper, Icarus cosimulation ($14$ vectors up to the $1024$-byte/$64$-beat maximum) plus a per-payload sweep over the RASD~§8.4 sizes (§\ref{sec:dma-sweep}), and a descriptor-oriented firmware driver; moved it from future work to implemented (§\ref{sec:dma}). Made the preamble engine-agnostic (pdf/Xe/Lua\LaTeX) and corrected a dangling table reference and a stray non-ASCII byte in a code listing.\\
\bottomrule
\end{tabular}
\end{center}
