...
module ascon_aead128_encrypt_kat_core(
  input  wire clk,
  input  wire rst_n,
  output reg  busy_o,
  output reg  done_o,
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      case (state_q)
        ST_INIT_LOAD: begin
          // S <- IV || K || N, packed as {x4,x3,x2,x1,x0} for state[319:0].
        ...
        ST_DOMAIN: begin
          // S <- S xor (0^319 || 1), i.e. toggle logical bit S[319].
          s_q <= s_q ^ {1'b1, 319'b0};
          state_q <= ST_PT_FULL;
        end
        ...
        ST_PT_P8: begin
          s_q <= round_state_w;
          activity_o <= 1'b1;
          if (rc_index_q == 4'd15) state_q <= ST_PT_FINAL;
          else rc_index_q <= rc_index_q + 4'd1;
        end
        ...
        ST_CHECK: begin
          if ((ct_q == CT_EXPECTED) && (~|tag_diff_w)) begin
            pass_o <= 1'b1;
            fail_o <= 1'b0;
          end else begin
            pass_o <= 1'b0;
